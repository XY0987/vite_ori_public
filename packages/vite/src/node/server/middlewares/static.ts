...
export function serveStaticMiddleware(
  ...
  return function viteServeStaticMiddleware(req, res, next) {
    /**
     * 这个中间件只处理“可以直接从 root 读出来”的静态文件。
     * HTML 要交给 indexHtmlMiddleware 做注入和 transform，/@vite、/@fs 等内部请求也要交给专门中间件。
     */
...
export function isFileLoadingAllowed(
  ...
  /**
   * server.fs.strict 是 dev server 的文件访问边界。
   * 开启后，只有显式 allow 的目录、Vite 认定安全的模块路径，
   * 且没有命中 deny glob 的文件，才允许被浏览器直接读取。
   */
...
export function checkLoadingAccess(
  ...
  if (isFileReadable(path)) {
    /**
     * 文件真实存在但不在 allow 范围内，说明这是越界文件访问，直接返回 403。
     * 如果文件不存在，则返回 fallback，让后续 API/mock/404 中间件继续判断。
     */
    return 'denied'
  }
