  ...
  const normalizedHotChannel = normalizeHotChannel(
    {
      send(payload) {
        /**
         * 核心逻辑：error/full-reload 在还没有浏览器连接时会被暂存。
         * 新客户端连上后仍能收到关键状态，避免启动早期的错误或刷新信号丢失。
         */
        if (
        ...
        /**
         * 核心逻辑：Vite HMR 通道只广播 JSON payload，不直接操作模块。
         * 浏览器端 client 收到 payload 后再决定重新 import、换 CSS link 或整页刷新。
         */
        const stringified = JSON.stringify(payload) // 🔖断点[小册08] WebSocket 广播:把 JSON 发给所有就绪客户端
