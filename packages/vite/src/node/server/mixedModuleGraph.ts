...
export class ModuleGraph {
  ...
  */
  ...
  getBackwardCompatibleModuleNodeDual( // 🔖断点[小册09] 老 ModuleNode 如何把 client/ssr 两张图的节点合成一个兼容视图
    clientModule?: EnvironmentModuleNode,
    ssrModule?: EnvironmentModuleNode,
  ): ModuleNode {
    /**
     * 兼容逻辑：旧版 ModuleNode 把 client/ssr 信息放在一个对象里；
     * Environment API 后真实节点拆成两张图，所以这里用双 WeakMap 缓存一个稳定的兼容视图。
     */
