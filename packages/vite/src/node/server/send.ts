...
export function send(
  ...
  /**
   * 核心逻辑：send 是 dev 响应的最后出口，所以这里再做一层 etag 304。
   * 即使前面的中间件没提前命中缓存，也能避免把同一份转换结果重复传给浏览器。
   */
  if (req.headers['if-none-match'] === etag) { // 🔖断点[小册05] 发送前最后一层 etag 304 兜底
    res.statusCode = 304
    res.end()
    return
  }
  ...
  /**
   * 核心逻辑：转换链路产出的 sourcemap 在发送前被追加到 JS/CSS 响应中。
   * 如果 JS 没有 map，也会补一个 fallback map，保证调试器能尽量映射回请求 URL。
   */
