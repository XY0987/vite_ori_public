...
export async function resolveEnvironmentPlugins(
  ...
  for (const plugin of environment.getTopLevelConfig().plugins) {
    if (plugin.applyToEnvironment) {
      /**
       * 核心逻辑：applyToEnvironment 是 Environment API 的插件级过滤/替换点。
       * 返回 false 表示该环境排除插件，返回 true 表示沿用原插件，返回插件对象则替换为环境专属实现。
       */
      const applied = await plugin.applyToEnvironment(environment) // 🔖断点[小册03/10] 环境维度过滤:返回 false=排除/true=保留/插件=替换
      if (!applied) {
        continue
      }
      if (applied !== true) {
