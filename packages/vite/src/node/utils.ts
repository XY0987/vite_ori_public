...
>(
  ...
  /**
   * 兼容逻辑：这是 Vite 8 从 Rollup 迁移到 Rolldown 后的历史 API 兼容层。
   * 外部仍可读写 rollupOptions，但内部统一通过 rolldownOptions 继续往下传。
   * 断点观察重点：只配置旧字段时如何迁移、两者同时存在时为什么新字段优先。
   */
  // if both rollupOptions and rolldownOptions are present,
  // ignore rollupOptions and use rolldownOptions
  buildConfig.rolldownOptions ??= buildConfig.rollupOptions // 🔖断点[专题02] rollupOptions↔rolldownOptions 兼容:老配置无感迁移,两者都写以 rolldownOptions 为准
  if (
  ...
  /**
   * 运行时把 rollupOptions 变成 rolldownOptions 的代理属性。
   * 后续代码即使继续读取或写入旧字段，也会落到新字段上，
   * 从而让老 API 入口和新内部实现共享同一份配置对象。
   */
  Object.defineProperty(buildConfig, 'rollupOptions', { // 🔖断点[小册11] rollupOptions↔rolldownOptions 兼容代理,老配置无感迁移
    get() {
