      ...
      async handler(code, id) {
        ...
        while ((match = re.exec(cleanString))) {
          ...
          if (rawUrl[0] === '`' && rawUrl.includes('${')) {
            /**
             * 用户写 `new URL(`./dir/${name}.png`, import.meta.url)` 时，
             * 构建期无法知道 name 的具体值，只能把可匹配的文件先展开成 glob 映射，
             * 运行时再用原始模板字符串作为 key 去取最终 URL。
             */
          ...
          if (!builtUrl) {
            ...
            /**
             * 文件在构建期找不到时不强行报错。
             * 例如用户希望运行时由 CDN、后端静态目录或浏览器原生 URL 规则解析，
             * Vite 会保留原表达式对应的 URL，并只给一次提示。
             */
