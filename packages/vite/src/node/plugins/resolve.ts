        ...
        const plugin = viteResolvePlugin({
          ...
          finalizeBareSpecifier: !depsOptimizerEnabled
            ...
            : (resolvedId, rawId, importer) => {
                ...
                /**
                 * 能走到这里，说明裸导入已经被解析成真实文件，但还没命中现成预构建产物。
                 * 接下来要决定：是跳过优化、直接给浏览器源码 URL；还是登记 missing import，
                 * 触发 optimizer 重新预构建，并把当前请求指向 .vite/deps 占位 id。
                 */
                ...
                if (skipOptimization) {
                  /**
                   * 跳过优化的 npm 依赖仍然加 browserHash。
                   * 这样锁文件或依赖优化结果没变时，浏览器能长期缓存这些源码 URL；
                   * 但只对 JS 类型加，避免 CSS/asset 这类外部化文件因为 query 产生重复模块。
                   */
                  ...
                  /**
                   * 这是运行时新发现的依赖。
                   * registerMissingImport 会安排下一轮 optimizeDeps，并先返回稳定的优化后 id，
                   * 让当前模块能继续完成 import URL 改写。
                   */
...
export function tryNodeResolve(
  ...
  if (
    ...
    return { id: resolved } // 小册说明：`scan` 是扫描阶段标记；Vite 8 的实际扫描器已经切换为 Rolldown 扫描器。
  }
  ...
  if (skipOptimization) {
    ...
    /**
     * 核心逻辑：运行时发现缺失依赖的入口在 resolve 阶段。
     * Vite 先登记 missing import，立刻返回未来的预构建产物路径；真正打包由 depsOptimizer 防抖合并执行。
     */
...
export async function tryOptimizedResolve(
  ...
  /**
   * 核心逻辑：preAlias 等早期插件需要知道某个 bare import 是否已优化。
   * 因此这里会等待扫描结束，避免在扫描结果未稳定时把依赖错误地解析回源码路径。
   */
