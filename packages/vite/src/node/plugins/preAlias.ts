...
export function preAliasPlugin(config: ResolvedConfig): Plugin {
  ...
  return {
    ...
    async resolveId(id, importer, options) {
      ...
      /**
       * 这个插件专门处理“命中 alias 的裸导入”。
       * 例如用户把 `react` alias 到某个兼容包时，要先确认它是否已经被预构建，
       * 避免后续 alias 插件直接把它解析回源码路径，绕开 depsOptimizer。
       */
      if (
        importer &&
        depsOptimizer &&
        ...
        if (findPatterns.find((pattern) => matches(pattern, id))) {
          ...
          if (resolved && !depsOptimizer.isOptimizedDepFile(resolved.id)) {
            ...
            if (
              ...
              /**
               * alias 后的真实文件仍然是可优化依赖，但本轮扫描还没发现它。
               * 这里先登记 missing import，并返回 .vite/deps 下的占位 id，
               * 后续 optimizer 重新跑完后浏览器请求会落到真实预构建产物。
               */
