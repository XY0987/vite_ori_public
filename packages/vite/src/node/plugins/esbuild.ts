...
export async function transformWithEsbuild( // 🔖断点[专题01] transformWithEsbuild 已弃用(保留兼容、惰性加载 esbuild),调用会打印 deprecation 警告
  code: string,
  filename: string,
...
export function esbuildPlugin(config: ResolvedConfig): Plugin { // 🔖断点[专题01] esbuildPlugin(vite:esbuild) 仅保留导出,Vite 8 已不在 plugins/index.ts 注册(被 vite:oxc 取代)
