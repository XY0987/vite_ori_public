...
async function bundleWorkerEntry(
  ...
  /**
   * worker 入口不会直接复用主构建的 bundle。
   * 这里会清理 URL、检查缓存和循环引用，然后为 worker 单独创建 BuildEnvironment。
   */
  ...
  /**
   * worker 使用独立的配置和插件实例，避免污染主构建环境。
   * 断点观察这里可以看到 workerConfig 如何由 bundleChain 派生出来。
   */
  ...
  /**
   * worker 子构建再次调用 Rolldown。
   * 它和主构建一样注入环境化插件、日志分派、CSS moduleTypes 和 viteMode，
   * 但 input/output 来自 worker 配置，并最终通过 generate 产出内存结果。
   */
