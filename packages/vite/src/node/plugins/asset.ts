...
export function assetPlugin(config: ResolvedConfig): Plugin {
  ...
  return {
    ...
    load: {
      ...
      async handler(id) { // 🔖断点[小册06] asset load:把 ?raw/?url/静态资源 import 转成 JS 模块
        /**
         * 核心逻辑：assetPlugin 把“资源 import”转换成 JS 模块。
         * ?raw 导出文件文本，?url/普通静态资源导出最终 URL，浏览器最终仍只加载 JS/URL。
         */
        ...
        /**
         * 核心逻辑：dev 期资源 URL 也要继承模块图里的 HMR 时间戳。
         * 这样图片等静态资源变更后，浏览器看到的是带新 t= 的 URL，不会被旧缓存挡住。
         */
