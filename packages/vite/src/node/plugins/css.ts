  ...
  return {
    ...
    transform: {
      ...
      async handler(raw, id) {
        ...
        /**
         * 核心逻辑：cssPlugin 只负责“编译 CSS”，不负责把 CSS 注入页面或产出文件。
         * 预处理器、PostCSS/LightningCSS、@import 和 url() 重写都会在 compileCSS 里完成。
         */
        const {
          code: css,
          modules,
          deps,
          map,
        } = await compileCSS( // 🔖断点[小册06] CSS 编译:预处理器 + PostCSS/LightningCSS + @import/url() 重写
          environment,
          id,
          raw,
          preprocessorWorkerController!,
        ...
        if (deps) {
          /**
           * 核心逻辑：CSS 编译过程中发现的 @import / 预处理依赖必须加入 watch。
           * 否则这些间接依赖变更时，dev server 无法触发对应 CSS 模块失效和 HMR。
           */
    ...
    transform: {
      ...
      async handler(css, id) {
        ...
        if (config.command === 'serve') {
          /**
           * 核心逻辑：dev 期浏览器不能直接 import CSS 模块，所以 cssPostPlugin 把 CSS 包装成 JS。
           * 这个 JS 模块负责调用 updateStyle，并默认注册 import.meta.hot.accept() 支持样式热更新。
           */
          ...
          const code = [
            ...
            `__vite__updateStyle(__vite__id, __vite__css)`, // 🔖断点[小册06] dev 期把编译好的 CSS 包成 JS:由 updateStyle 往 <head> 插 <style>(所以 CSS 默认就有 HMR)
        ...
        /**
         * 核心逻辑：build 期不再把普通 CSS 注入页面，而是先收集到 styles，
         * 后续 generateBundle 再按 chunk/code-splitting 策略抽成最终 CSS 资源。
         */
        // build CSS handling ----------------------------------------------------
  ...
  return {
    ...
    transform: {
      ...
      handler(_, id) {
        ...
        /**
         * 核心逻辑：CSS 的依赖图和 HMR 接受关系由 css-analysis 维护。
         * JS import 边在 importAnalysis 中更新；CSS @import / addWatchFile 产生的文件依赖在这里补进模块图。
         */
        ...
        if (thisModule) {
          ...
          if (pluginImports) {
            ...
            moduleGraph.updateModuleInfo( // 🔖断点[小册06] css-analysis 把 CSS @import/addWatchFile 依赖补进模块图
              thisModule,
              depModules,
              null,
