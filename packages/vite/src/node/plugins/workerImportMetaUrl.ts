    ...
    transform: {
      ...
      async handler(code, id) {
        ...
        while ((match = re.exec(cleanString))) {
          ...
          /**
           * import.meta.url 里的 worker URL 最终要落到真实文件：
           * 相对路径走文件系统解析，裸路径/绝对路径则复用 Vite resolver 或 publicDir 兜底。
           */
          if (url[0] === '.') {
          ...
          if (
            ...
            if (isBundled) {
              ...
              /**
               * 非 bundled 模式不提前打包 worker，而是把 URL 标记成
               * ?worker_file&type=...，后续 workerPlugin 会按请求再处理这个入口。
               */
