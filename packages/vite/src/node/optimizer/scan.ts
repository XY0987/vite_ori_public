...
export function devToScanEnvironment(
  ...
  /**
   * 依赖扫描只需要“解析能力”，不应该访问完整 dev server 能力。
   * 例如扫描 `import vue from 'vue'` 时，只复用插件容器的 resolveId，
   * 不建立模块图节点，也不触发 HMR、文件监听等运行期副作用。
   */
  return {
    mode: 'scan',
    get name() {
...
export function scanImports(environment: ScanEnvironment): {
  ...
  async function scan() {
    /**
     * 扫描的目标不是产出 bundle，而是从入口模块一路爬 import，
     * 找出需要预构建的裸导入依赖，例如 `vue`、`react`、`lodash-es`。
     */
...
async function computeEntries(environment: ScanEnvironment) {
  ...
  /**
   * 入口来源优先级：
   * 1. 用户显式配置 optimizeDeps.entries，完全按用户指定扫描；
   * 2. 否则复用 build.rolldownOptions.input，保证 dev 预扫描和 build 入口一致；
   * 3. 再没有才回退到项目里的 HTML 入口。
   */
...
async function prepareRolldownScanner(
  ...
  /**
   * 核心逻辑：依赖扫描并不是一套独立解析器，它仍然走 Rolldown 插件管线。
   * rolldownScanPlugin 内部会复用当前 environment 的 resolve 规则，保证扫描结果和 dev 请求解析保持一致。
   */
  plugins.push(
    ...rolldownScanPlugin(
      environment,
...

    {
      ...
      async resolveId(id, importer) {
        ...
        if (/^[\w@][^:]/.test(id)) {
          /**
           * 这里是依赖扫描最关键的分叉：
           * - 解析到 node_modules：记录进预构建列表，然后 externalize，停止继续爬它的内部依赖；
           * - 解析到源码/软链接包：继续把它当源码向下扫描，避免 monorepo 包被误当成普通依赖。
           */
