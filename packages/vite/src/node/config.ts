...
export function resolveDevEnvironmentOptions(
  ...
  /**
   * 用户通常只配置 environment 的行为，不需要提供 createEnvironment。
   * 这里把原始 DevEnvironmentOptions 归一化成 ResolvedDevEnvironmentOptions：
   * client 默认创建浏览器 DevEnvironment，其它环境默认创建 RunnableDevEnvironment。
   * mergeWithDefaults 仍允许 Worker 等高级场景用 dev.createEnvironment 覆盖默认工厂。
   */
...
function resolveEnvironmentOptions(
  ...
  /**
   * 核心逻辑：环境配置在这里决定运行身份。
   * consumer 决定解析条件和运行端(client/server)，isBundled 区分 build/bundled dev，后续插件和模块图都按这些环境属性分流。
   */
  const consumer =
  ...
  if (
    ...
    ;(resolvedConfig.optimizeDeps.rolldownOptions.plugins as any[]).push(
      ...resolvedConfig.optimizeDeps.esbuildOptions.plugins.map((plugin) =>
        convertEsbuildPluginToRolldownPlugin(plugin), // 🔖断点[专题04] 老 esbuild 预构建插件被翻译成 Rolldown 插件(兼容垫片,覆盖常用钩子)
      ),
    )
  }
}
...
export async function resolveConfig(
  ...
  /**
   * 核心逻辑：resolveConfig 是 Vite 配置归一化总入口。
   * 它不是简单读 vite.config，而是要把 CLI/API 配置、配置文件、插件钩子、默认值、兼容层和环境模型都合成最终 ResolvedConfig。
   */
  let config = inlineConfig
  /**
   * Vite 8 内部已经以 rolldownOptions 为主，但生态里还有大量 rollupOptions。
   * 这里先给 build/worker/optimizeDeps/ssr.optimizeDeps 挂兼容代理，保证后续读取统一落到 rolldownOptions。
   */
  config.build ??= {}
  ...
  /**
   * mode 是 Vite 自己的模式；NODE_ENV 会影响依赖包的条件分支。
   * 所以在加载用户配置前就要尽早确定 NODE_ENV，避免配置执行时读到不一致的环境。
   */
  ...
  /**
   * 核心逻辑：resolveConfig 的第一段先把“用户写的配置”读进来。
   * 后面所有默认值补齐、插件排序和环境归一化，都基于这份合并后的 config 继续推进。
   */
  ...
  if (configFile !== false) {
    ...
    if (loadResult) {
      config = mergeConfig(loadResult.config, config) // 🔖断点[小册02] 文件配置与命令行/API 配置合并(第二个参数=命令行,胜出)
  ...
  // apply 只决定插件是否进入当前 command；真正的位置排序由 enforce 在下一步决定。
  ...
  /**
   * 插件主线：先过滤 apply，再按 enforce 分桶，跑 config 钩子，最后插入 Vite 内部插件模板。
   */
  const rawPlugins = (await asyncFlatten(config.plugins || [])).filter( // 🔖断点[小册03] 拍平+按 apply 过滤后的用户插件 rawPlugins
    filterPlugin,
  )

  const [prePlugins, normalPlugins, postPlugins] = sortUserPlugins(rawPlugins) // 🔖断点[小册03] 按 enforce 分成 pre/normal/post 三桶

  const isBuild = command === 'build'

  // run config hooks
  const userPlugins = [...prePlugins, ...normalPlugins, ...postPlugins]
  config = await runConfigHook(config, userPlugins, configEnv) // 🔖断点[小册02/03] 跑用户插件的 config 钩子(仅用户插件,串行 merge 改配置)

  /**
   * Environment API 的默认约束：dev 期始终准备 client/ssr 两个环境，build 期按配置决定是否构建 ssr。
   */
  ...
  /**
   * 生态迁移提示：Vite 已内置 tsconfig paths 解析，但老项目可能还装着插件。
   * 这里不改变行为，只给出 warnOnce，避免用户误以为两个解析器会叠加增强。
   */
  ...
  /**
   * 从这里开始把旧的顶层配置迁移/回填到 Environment API。
   * 读源码时可以把它理解为“兼容旧配置入口，同时建立新环境模型”的桥接层。
   */
   ...
   * 旧 API 是按 clientFiles/ssrFiles 分组；新模型把 warmup 挂到各自 environment.dev 下。
   */
   ...
   * 顶层 ssr 是旧入口；Environment API 下真正生效的是 environments.ssr。
   * 这里把旧 ssr 配置当默认值合进去，避免老插件/老项目升级后行为断裂。
   */
   ...
   * client 继承顶层 resolve/optimizeDeps；非 client 环境不继承 createEnvironment/warmup 这类 client 专属默认值。
   */
  ...
  for (const name of Object.keys(config.environments)) {
    config.environments[name] = mergeConfig( // 🔖断点[小册02] 环境默认配置与用户环境配置合并(client 继承顶层配置,ssr/自定义环境使用环境默认值)
      name === 'client'
  ...
  /**
   * configEnvironment 钩子允许插件在“每个环境”维度继续调整配置，
   * 它发生在默认环境配置合并之后、ResolvedEnvironmentOptions 归一化之前。
   */
   ...
   * 很多插件仍读取 config.resolve；所以 client 环境归一化后还要回填顶层视图。
   */
  config.resolve ??= {}
  ...
  /**
   * 这里才把每个 environment 的用户配置变成 ResolvedEnvironmentOptions。
   * 后续 dev server/build 都按环境读取自己的 resolve、optimizeDeps、dev/build 配置。
   */
  ...
  /**
   * load .env files
   * envFile 是旧开关；新语义用 envDir=false 表示彻底不加载 .env。
   * loadEnv 的结果稍后会和 BASE_URL/MODE/DEV/PROD 一起挂到 resolved.env。
   */
   ...
   * base 在 dev、build、SSR build 下语义不同：dev 不能使用相对 base，SSR 也不能依赖 import.meta.url 推导。
   */
  ...
  /**
   * worker.plugins 曾经允许直接写数组；现在改成函数，避免主构建和 worker 构建共享同一批插件实例状态。
   */
  ...
  const createWorkerPlugins = async function (bundleChain: string[]) {
    /**
     * Worker 构建会再次跑一套插件解析，但基于主配置派生 workerConfig。
     * 这样 worker 可以复用大部分 Vite 默认能力，又不会污染主构建插件状态。
     */
  ...
  /**
   * Vite 8 默认转译器从 esbuild 切到 Oxc。
   * 这里保留 esbuild 配置到 oxc 配置的兼容转换，同时提醒 esbuild:false 不再关闭默认转译。
   */
  ...
  if (config.esbuild) {
    if (config.oxc) {
      ...
      oxc = convertEsbuildConfigToOxcConfig(config.esbuild, logger) // 🔖断点[专题03] 老 esbuild 配置自动转成 oxc 配置;注意 esbuild:false 已失效,要写 oxc:false
    }
  ...
  /**
   * 先构造内部真正使用的 resolved，再把用户 config 覆盖进来保留外部可见字段。
   * 注意 plugins 这里还是占位，下面 resolvePlugins 后才替换成完整内部插件链。
   */
  resolved = { // 🔖断点[小册02] 组装最终 ResolvedConfig(逐字段套用默认值、兼容层和 resolve*Options 结果)
  ...
  /**
   * 到这里 ResolvedConfig 形状已基本稳定，内部插件才能读取最终配置并完成装配。
   */
  ...
  await Promise.all( // 🔖断点[小册02] configResolved 钩子(此时全部插件就位,拿到的是定稿 ResolvedConfig)
    resolved
      .getSortedPluginHooks('configResolved')
   ...
   * downstream projects modifying the plugins in it. This may change
   * once the ecosystem is ready.
   * 环境插件列表必须晚于 configResolved，因为生态插件仍可能在 configResolved 里改 resolved.plugins。
   */
...
export function sortUserPlugins(
  ...
  /**
   * 核心逻辑：这里仅按 plugin.enforce 做宏观分桶。
   * 单个 hook 内的 order: 'pre'/'post' 会在 getSortedPluginsByHook 再排序，两层排序彼此正交。
   */
...
async function bundleConfigFile(
  ...
  const bundle = await rolldown({ // 🔖断点[小册02] 用 Rolldown 打包 vite.config(externalize 依赖、注入 __dirname 等)
    input: fileName,
...
async function loadConfigFromBundledFile(
  ...
  if (isESM) {
    ...
    try {
      return (await import(pathToFileURL(tempFileName).href)).default // 🔖断点[小册02] 动态 import 临时 .mjs 执行配置,拿到 default 导出
    } finally {
...
async function runConfigHook(
  ...
  /**
   * 核心逻辑：config 钩子只跑用户插件，并且发生在 Vite 内部插件装配之前。
   * 每个插件返回的配置会 merge 到当前 conf，后返回的插件配置可以覆盖前面的结果。
   */
  ...
  for (const p of getSortedPluginsByHook('config', plugins)) { // 🔖断点[小册03] config 钩子按 hook.order 排序后串行执行，返回对象再依次合并
    const hook = p.config
...
async function runConfigEnvironmentHook(
  ...
  /**
   * 核心逻辑：configEnvironment 是 Environment API 的配置阶段钩子。
   * 它在默认环境配置 merge 之后、resolveEnvironmentOptions 之前执行，可以按环境名分别改 client/ssr/custom 配置。
   */
  const context = new BasicMinimalPluginContext<
    Omit<PluginContextMeta, 'watchMode'>
