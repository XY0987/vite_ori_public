...
/**
 * 浏览器端 HMR 消息总入口。
 * 服务端通过 WebSocket 发来的只是 payload 指令；这里按 type 分发 update、full-reload、
 * prune、error 等消息，再把 JS 更新交给 HMRClient runtime，把 link CSS 更新在 DOM 中换标签。
 */
async function handleMessage(payload: HotPayload) { // 🔖断点[小册08·客户端] 浏览器收到所有 WS 消息的入口(需 sourcemap)
  switch (payload.type) {
    case 'connected':
    ...
    case 'update':
      ...
      /**
       * 一个 update payload 里可能同时包含多个 js/css 更新。
       * JS 更新交给 HMRClient 重新 import 并执行 accept 回调；link CSS 则直接换标签，避免走模块图。
       */
      await Promise.all(
        payload.updates.map(async (update): Promise<void> => {
          if (update.type === 'js-update') { // 🔖断点[小册08·客户端] JS 更新交给 HMRClient 队列重新 import 并执行 accept
            return hmrClient.queueUpdate(update)
          }

          /**
           * css-update 只处理由 <link> 直接引用的样式。
           * 通过 clone 新 link 并等待 load 后移除旧 link，避免直接改 href 造成短暂无样式闪烁。
           */
          const { path, timestamp } = update // 🔖断点[小册08·客户端] CSS link 更新通过 clone 新 link 避免直接改 href 闪烁
